...
public class Solution {
    ...
    public UndirectedGraphNode cloneGraph(UndirectedGraphNode node) {
        ...
        // copy neighbors
        for (UndirectedGraphNode n : nodes) {
            UndirectedGraphNode cpNode = map.get(n);
            for (UndirectedGraphNode nb : n.neighbors) {
                UndirectedGraphNode cpNb = map.get(nb);
                cpNode.neighbors.add(cpNb);
            }
        }
    ...
    private List<UndirectedGraphNode> getAllNodes(UndirectedGraphNode node) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (UndirectedGraphNode curtNb : curt.neighbors) {
                if (visited.contains(curtNb)) {
                    continue;
                }
                queue.offer(curtNb);
                visited.add(curtNb);
